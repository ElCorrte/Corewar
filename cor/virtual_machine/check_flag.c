...
/*   Updated: 2017/10/27 14:54:39 by yzakharc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	n(char **av, int *i, t_skrr *skrr, int ac)
{
	...
	cnt = skrr->max_player;
	*i + 1 < ac ? skrr->flag_n[cnt] = (int)ft_atoi(av[*i + 1]) : 0;
	skrr->flag_n[cnt] < 0 || !av[*i + 1] ? argv_error(0, 0, 8) : 0;
	skrr->flag_n[cnt] != 0 ? (*i) += 2 : argv_error(0, 0, 8);
}
